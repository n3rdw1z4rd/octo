#include "../include/core/dynamic_array.h"
...
void *_darray_create(u64 length, u64 stride)
{
    ...
    u64 *new_array = calloc(header_size + array_size, sizeof(u64));
...
void *_darray_pop_at(void *array, u64 index, void *dest)
{
    ...
    if (index >= length)
    {
        LOG_ERROR("darray_pop_at: index outside of bounds: length: %i, index: %i", length, index);
        return array;
    }
...
void *_darray_insert_at(void *array, u64 index, void *value_ptr)
{
    ...
    if (index >= length)
    {
        LOG_ERROR("darray_insert_at: index outside of bounds: length: %i, index: %i", length, index);
        return array;
    }
